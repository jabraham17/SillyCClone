...
void symbolListToDot(char* name, symbol_list_t* sym_list, FILE* f) {
    ...
    while(temp != NULL) {
        fprintf(
            ...
            (intptr_t)(temp->symbol), temp->symbol->name, symbolColors(temp->symbol));
        fprintf(
...
void statementToDot(ast_stmt_t* stmt, FILE* f) {
    ...
    switch(stmt->type) {
        ...
        case ast_SYMBOL:
            fprintf(f, "[label=\"%s\";%s];\n", stmt->data.symbol->name, symbolColors(stmt->data.symbol));
            break;
        case ast_EQUALS:
        case ast_DEQUALS:
        case ast_PLUS:
...
void functionToDot(function_t* func, FILE* f) {
    ...
    fprintf(f, "func_%ld[label=\"%s\";%s];\n", (intptr_t)func, func->symbol->name, symbolColors(func->symbol));
